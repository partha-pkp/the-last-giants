...
pred <- ggplot(ols_predictions_data, aes(x = Year, y = Population, group = 1)) +
  geom_line(aes(color = ifelse(Year <= 2014, "Blue", "Red"))) +
  stat_smooth(method = "loess", se = FALSE) +
  ggtitle("Combined Data: Estimate Over Time") +
  xlab("Year") +
...
```{r}
#| fig-alt: Visualization of Ivory exports in Africa from 1990 to 2022
...
exports_barplot <- ggplot(export_count_per_year, aes(x = as.factor(Year), y = export_count, fill = as.factor(Year), text = paste("Export count: ", export_count))) +
  ...
  labs(
    x = "Year",
    y = "Number of exports",
    title = "Ivory exports in Africa from 2000 to 2022",
...
gg_export_countries <- ggplot() +
  ...
  theme_void() +
  labs(title = "Ivory exports from 1995 to 2022", caption = "Source: CITES (Convention of International Trade 
    in Endangered Species of Wild Fauna and Flora)")
